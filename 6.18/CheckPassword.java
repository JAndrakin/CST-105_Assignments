	...
	public static boolean chkPswd(String password) {
		...
		if (password.length() < 8) {
			...
			for (int i = 0; i < password.length(); i++) {
				...
				} else { //if a char in the password is not a letter or a digit, set chkPswd to false and break loop
					chkPswd = false;
					break;
				}
		}
		
		if (numberOfDigit < 2) {//A password must contain at least two digits.
			chkPswd = false;
		}
