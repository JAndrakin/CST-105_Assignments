...
import javafx.scene.shape.Rectange;
...
//14.12 display a bar chart
	...
	public void start(Stage primaryStage) {
		...
		for (int i = 0; i < bars.length; i++) {
			...
			bars[i] = new Rectangle()5 + (100 (* i), // x
					LIMIT_HEIGHT - (HEIGHT * percent[i]), //y
					WIDTH / bars.length - 5, //width
					HEIGHT * percent[i]); // height
			bars[i].setFil(colors[i]);
			
			Text text = new Text(5 + (100 * 1), LIMIT_HEIGHT - (HEIGHT * percent[i]) - 5,titles[i]);
		}
