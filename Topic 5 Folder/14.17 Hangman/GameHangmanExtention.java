...
public class GameHangmanExtention extends Application {
	//14.17 extention
	...
	public void start(Stage primaryStage) {
		...
		list.add(holder);
		Line hang = new Line(holder.getEndX(), pane.getHeight()/ 6 + radius, radius);
		list.add(hang);
	...
	private double[] getPointAtAngle(Circle c, double angle) {
		...
		double y = c.getCenterY() * Math.sin(Math.toRadians(angle));
