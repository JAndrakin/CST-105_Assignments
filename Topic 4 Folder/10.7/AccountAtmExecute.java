...
public class AccountAtmExecute {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < accounts.length; i++) {
			accounts[i] = new AccountAtm(i, 100);
		}
	}
	...
	while (option != 4) {
		Account user = getAccountAtm(accounts, id);
...
public static Account getAccountAtm(AccountAtm[] accounts, int id) {
	
	for (AccountAtm account : accounts) {
		if (id == account.getId()) {
			return account;
		}
	}
